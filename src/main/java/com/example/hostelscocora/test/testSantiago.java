...
public class testSantiago {
    ...
    public static void main(String[] args) {
        ...
        if (overlaps) {
            ...
            System.out.println("Ranges do not overlap.");
        }
    }

    private static boolean verificarSuperPone(List<Fecha> ranges) {
        for (int i = 0; i < ranges.size(); i++) {
            for (int j = i + 1; j < ranges.size(); j++) {
                if (ranges.get(i).isSuperpone(ranges.get(j))) {
                    return true; // Se encontró superposición
                }
            }
        }
